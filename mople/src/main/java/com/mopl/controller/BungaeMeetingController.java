...
public class BungaeMeetingController {
		...
		public ModelAndView bungaeMeetingList(HttpServletRequest req, HttpServletResponse resp)
			...
			try {
				...
				List<BungaeMeetingDTO> list = dao.findAllMeetings(offset, size, sportCategory, regionCategory, sortBy);
		...
		// 번개모임 리스트 - AJAX:TEXT
		@RequestMapping(value = "/bungaeMeeting/bungaeMeetingLayout", method = RequestMethod.GET)
		public ModelAndView meetingLayout(HttpServletRequest req, HttpServletResponse resp)
				throws ServletException, IOException {
			ModelAndView mav = new ModelAndView("bungaeMeeting/bungaeMeetingLayout");
	...
	// 번개모임 생성 폼
		...
		public ModelAndView meetingCreateSubmit(HttpServletRequest req, HttpServletResponse resp)
			...
			return new ModelAndView("redirect:/bungaeMeeting/bungaeMeetingList?sportCategory=0&regionCategory=0");
		}
		...
		public Map<String, Object> approveMember(HttpServletRequest req, HttpServletResponse resp) {
			...
			try {
				...
				// dao.updateMemberRole(dto); -> 만들어야함
